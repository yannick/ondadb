...
| M3 | **Still deferred, and now measured.** The latency coupling is real, but dropping `commit_mu` around WAL/apply would break the validation-to-apply exclusion unless the engine first gains write intents or a second validation/publication protocol. That protocol change was too broad for a corrective release and remains too broad for 0.9. | No correctness failure exists under the current locking; this remains a performance design item — but 3.2 attached a price to it. Extending `commit_mu` over a prepared-transaction reservation check is free at one thread (p50 8,230 vs 8,208 ns, inside a 41-54% within-arm spread over 6 runs each) and a ~3x throughput regression at eight (2,206 -> 730 ops/sec median, p99 20 -> 83 ms, disjoint distributions). Evidence in `bench-results/3.2/2026-08-30/`. |
...
| M5 | **Resolved in 0.9.0** by feature 2.2. `MANIFEST-EDITS` is an append-only log of numbered, CRC-framed catalog edits with periodic snapshot compaction, behind `CAP_MANIFEST_EDITS`; every catalog mutation now goes through `DbInner::catalog_txn`, which makes the edit durable before it publishes. Without the capability the full rewrite is unchanged, byte for byte. | `tests/manifest_edits.rs` (including an inventory guard that fails closed when a new `SstMeta` field is not carried through the edit codec), plus the crash matrix in that file. The `manifest_encoded_size_at_scale` probe remains the sizing evidence for why. |
